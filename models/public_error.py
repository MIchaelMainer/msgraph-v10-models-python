...
from ..model.public_inner_error import PublicInnerError
from ..one_drive_object_base import OneDriveObjectBase


class PublicError(OneDriveObjectBase):
    ...
    def inner_error(self):
        """
        Gets and sets the innerError
        
        Returns: 
            :class:`PublicInnerError<onedrivesdk.model.public_inner_error.PublicInnerError>`:
                The innerError
        """
        if "innerError" in self._prop_dict:
            if isinstance(self._prop_dict["innerError"], OneDriveObjectBase):
                return self._prop_dict["innerError"]
            else :
                self._prop_dict["innerError"] = PublicInnerError(self._prop_dict["innerError"])
                return self._prop_dict["innerError"]

        return None

    @inner_error.setter
    def inner_error(self, val):
        self._prop_dict["innerError"] = val
